...
const experts = [
  {
    name: "اميرة قصي",
    imageUrl: process.env.PUBLIC_URL + "/img/amera.png",
    role: "خبير في المنظمات الغير ربحية",
    bio: "",
  },
  {
    name: "موسى الموسى",
    imageUrl: process.env.PUBLIC_URL + "/img/musa.png",
    role: "مؤسس مشروع لتأهيل الباحثين عن عمل.",
    bio: "",
  },
  {
    name: "عبدالعزيز المرشدي",
    imageUrl: process.env.PUBLIC_URL + "/img/abdalziz.png",
...
const Expert = () => {
  ...
  const filteredExperts = experts.filter(
    (expert) =>
      ...
      (filterOption === "all" ||
        expert.role.toLowerCase().includes(filterOption.toLowerCase()))
  );
  ...
  return (
    <div className="relative">
      {/* Background Image */}
      <img
        src={process.env.PUBLIC_URL + "/Services.svg"}
        alt="Background"
      ...
      <div className="relative z-10 bg-gray py-24 sm:py-32">
        ...
        <div className="h-14 flex-col justify-start items-center gap-2 flex">
          <div className="text-slate-900 text-base font-normal leading-tight">
            حجز جلسة مع خبير
          </div>
          <div className="w-96 text-center text-zinc-900 text-2xl font-bold leading-normal">
            خطواتك الأولى نحو الاستشارة والتوجيه الفعّال.
          </div>
        </div>
        ...
        {/* Experts Section */}
        <div
          className="mx-auto grid max-w-3xl grid-cols-1 gap-x-8 gap-y-16 border-t border-gray-200 pt-10 sm:pt-16 lg:grid-cols-3"
          dir="rtl"
        >
          {filteredExperts.map((expert, index) => (
            <div
              key={index}
              className="px-6 py-12 bg-[#69B1B0] rounded-xl flex-col justify-center items-center gap-7 inline-flex"
            >
              <img
              ...
              <div className="flex-col justify-center items-center gap-3">
                <div className="w-64 text-center text-zinc-900 text-2xl font-bold leading-normal">
                  {expert.name}
                </div>
                <div className="w-64 text-center text-white text-sm leading-tight">
                  {expert.role}
                </div>
              </div>
              <div className="px-4 py-2 bg-neutral-100 rounded-md justify-center items-center gap-2.5 inline-flex">
                <a
                  className="text-black/opacity-50 text-base leading-tight"
        ...
        <div className="flex justify-center space-x-4 mt-4">
          <div className="w-auto h-11 px-5 py-3 rounded-md border border-zinc-500/opacity-30 justify-center items-center gap-3 inline-flex">
            <button className="text-zinc-500 text-base leading-tight">
              رؤية كل الخبراء
            </button>
          </div>
        </div>
